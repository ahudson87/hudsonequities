...
class WP_Yelp_Review_Admin {
	...
	public function wpyelp_yelp_section_developers_cb($args)
	{
		...
		echo "<p>Use this page to download your Yelp business reviews (Pro Version allows for multiple Yelp businesses) and save them in your Wordpress database. Please note that the plugin can only return recommended reviews. They will show up on the Review List page once downloaded. There are a couple of rules that Yelp has for their reviews.</p>
		<ul>
		...
		(The Pro Version allows you to grab reviews from up to 15 different businesses.)";
	}
			...
			if (filter_var($options['yelp_business_url'], FILTER_VALIDATE_URL)) {
			  ...
			  if($options['yelp_radio']=='yes'){
				...
				// Create DOM from URL or file
					...
					//$html = str_get_html($fileurlcontents);
	...
	//} else {
	//		$errormsg = $errormsg . ' <p style="color: #A00;">fopen is not allowed on this host. Please ask your hosting provided to turn it on.</p>';
	//		$this->errormsg = $errormsg;
	//	}
